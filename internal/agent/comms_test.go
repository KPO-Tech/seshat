...
func newDispatcher(t *testing.T) (*agent.Dispatcher, *agent.ProfileRegistry, *mailbox.SQLiteMailbox) {
	t.Helper()
...
func TestDispatcher_Broadcast(t *testing.T) {
	...
	// Register two agents in the same team.
	...
	// Nexus broadcasts to alpha team (excluding itself).
	...
	// Nexus must not receive its own broadcast.
...
func TestDispatcher_Assign_ByRole(t *testing.T) {
	...
	// Find the built-in researcher (Aria).
...
func TestDispatcher_Assign_ByRoleAndTeam(t *testing.T) {
	...
	// Two researchers in different teams.
	...
	// Assign to "researcher" scoped to "beta" team — only Faouziath should receive it.
	require.NoError(t, d.Assign(ctx, "system", "researcher", "beta", "Beta research task", ""))
...
func TestDispatcher_Assign_NoMatch(t *testing.T) {
	...
	require.Error(t, err)
	var noAgent *agent.ErrNoAgentForRole
...
func TestTeamBus_DeliversMessages(t *testing.T) {
	...
	bus := agent.NewTeamBus(reg, mb, handler, 50*time.Millisecond)
	require.NoError(t, bus.Start(ctx))
	t.Cleanup(bus.Stop)

	// Send a message to the built-in researcher (Aria).
	researchers, err := reg.FindByRole(ctx, "researcher")
	require.NoError(t, err)
	require.Len(t, researchers, 1)
	aria := researchers[0]

	msg := mailbox.NewMessage(mailbox.KindTask, "system", aria.ID, "Research task", "Go generics.")
	require.NoError(t, mb.Send(ctx, msg))

	// Wait up to 500ms for the bus to pick it up.
	...
	assert.Equal(t, "Research task", received[0].Subject)
}
...
func TestTeamBus_Start_Idempotent(t *testing.T) {
	...
	bus := agent.NewTeamBus(reg, mb, nil, 50*time.Millisecond)
	require.NoError(t, bus.Start(ctx))
