...
func NewHNSWStore(dir string) (Store, error) {
	return nil, fmt.Errorf("the hnsw vector store is not available on Windows (github.com/coder/hnsw's atomic-write dependency doesn't support this OS) — use vector.StoreSQLite or vector.StoreMemory instead")
}
