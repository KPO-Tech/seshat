...
// internal/deepdoc/parser/pdf/pdfsync/, licensed under the Apache License,
// Version 2.0 (see https://github.com/infiniflow/ragflow/blob/main/LICENSE).
// Copyright the RAGFlow/InfiniFlow authors.
//
// Unconfirmed for this port: whether the standalone published
// github.com/yfedoseev/pdf_oxide/go module also links libpdfium internally
// the way RAGFlow's own vendored build does (their symbol-collapsing comment
// above is specific to their build.sh). What's certain regardless: PDFium
// itself is documented non-thread-safe, so every pdfium.go call in this repo
// must go through Mu. Whether pdf_oxide calls also need to share it is not
// yet verified here - do not assume either way without testing concurrent
// pdf_oxide + pdfium calls together first.
package pdfsync
