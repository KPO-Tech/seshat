...
// EnvSharedLibraryPath, when set, is passed to onnxruntime_go's
// SetSharedLibraryPath before initializing - the only way to point
// InitORT at a specific onnxruntime.{so,dylib,dll} without a build-time
// static link (see WeightSharingEnabled's own doc comment for when that
// applies). Windows in particular needs this in practice: LoadLibrary's
// search order checks the running executable's own directory and the
// system directory before PATH, so if the host already has some other
// onnxruntime.dll reachable there (a stray copy from an unrelated
// ML/Python install, for example - confirmed hitting exactly this while
// validating Windows support), the wrong one wins silently unless the path
// is given explicitly.
const EnvSharedLibraryPath = "SESHAT_NATIVEDOC_ONNXRUNTIME_PATH"
...
// WeightSharingEnabled gates the sharedWeights optimization below. Default
// false - deliberately NOT auto-detected or attempted-then-caught, because
// there is nothing safe to catch: GetInitializerCount/GetInitializerName/
// GetInitializer call three OrtApi function-table slots
// (SessionGetInitializerCount / SessionGetInitializerName /
// SessionGetInitializer) that only exist in a custom patch RAGFlow builds
// into their own onnxruntime static library
// (infiniflow/ragflow-build, tag onnxruntime-v1.29.0-patch1 at the time
// this was ported). Calling them against a stock Microsoft onnxruntime
// build - which is what Windows gets, since that patch currently publishes
// no Windows asset, and what any !static Linux/macOS build gets unless it
// specifically fetched the patched one - doesn't return a Go error to
// gracefully degrade from: it's a call through a null function pointer,
// which crashes the process (confirmed: reproduced this exact crash while
// validating Windows support - see
// docs/issues/document-intelligence-roadmap.md phase 1.W). Set this to true
// only when you know for certain the linked onnxruntime is the patched
// build.
var WeightSharingEnabled = false
...
// initializers worth sharing, or when WeightSharingEnabled is false (see its
// own doc comment - this is not a safe thing to attempt speculatively).
